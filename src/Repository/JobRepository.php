...

{
    ...
    public function findJobsInProgressOrClosedByUser(User $user, $inProgress = true): ?array
    {
        ...
        $sql = "SELECT j.id, j.title, j.created_at, j.recruiter, a.name as action_name, a.set_closed, MAX(jt.created_at) AS maxCreatedAt, j.offer_description AS description, DATEDIFF( CURRENT_DATE, MAX(jt.created_at) ) AS delai FROM job j inner JOIN job_tracking jt ON jt.job_id = j.id inner JOIN action a ON a.id = jt.action_id WHERE j.user_id =  :user AND ( jt.created_at = ( SELECT MAX(jt2.created_at) FROM job_tracking jt2 WHERE jt2.job_id = j.id ) ) AND ( (:inProgress = 1 AND (jt.id IS NULL OR a.set_closed = 0 OR a.set_closed IS NULL)) OR (:inProgress = 0 AND (jt.id IS NOT NULL AND a.set_closed = 1)) ) GROUP BY j.id, a.name, a.set_closed, j.title, j.created_at, j.recruiter ORDER BY delai DESC;";
